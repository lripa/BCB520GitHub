...
# Assuming 'participants' and 'applicants_summary' have been prepared as per your previous code
...
# Adjusting the size of the circles by reducing the multiplier or changing the formula
p <- plot_geo(applicants_summary, locations = ~CODE, text = ~paste(Country, '<br>Applicants: ', Applicants),
              marker = list(size = ~sqrt(Applicants) * 2,  # Reduced multiplier for smaller circles
                            color = ~Applicants, colorscale = 'Reds', 
                            line = list(color = "#d1d1d1", width = 0.5))) %>%
